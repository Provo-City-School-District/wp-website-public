<aside id="rightSidebar" class="rightSidebar">
	<ul class="imagelist">
		<li>
			<a href="https://provo.edu/provo-adult-education/provo-adult-education-calendar/">
				<img src="//globalassets.provo.edu/image/icons/calendar-lt.svg" alt="" />
				<span>Adult Ed Calendar</span>
			</a>
		</li>
		<li>
			<a href="https://provo.edu/provo-adult-education/english-language-acquisition-calendar/">
				<img src="//globalassets.provo.edu/image/icons/calendar-lt.svg" alt="" />
				<span>ELA Calendar</span>
			</a>
		</li>
	</ul>	
	<h2>Contact Provo Adult Education</h2>
	<?php
	echo file_get_contents('https://provo.edu/directory_page/provo-adult-education/');
	?>

	<!--		
		
		I changed this content out for what's available on 02-18-2020 for WO54222

		
		
		<ul>

			<li><strong>Adult High School</strong></li>
			<li>636 North Independence Ave.<br />Provo, Utah 84601<br /><a class="ext" href="https://www.google.com/maps/dir//636+Independence+Ave,+Provo,+UT+84601/@40.2423912,-111.6892565,17z/data=!3m1!4b1!4m9!4m8!1m0!1m5!1m1!1s0x874d9a1656b47399:0x52b92f1d411f3292!2m2!1d-111.6870678!2d40.2423912!3e1">Get Directions to PAE</a><br />801-374-4840</li>
			<li><strong>Adult ELA Program</strong></li>
			<li>750 West 200 North<br />Provo, Utah 84601<br />801-374-4904</li>
		</ul>
	<h2>Adult High School Office Hours</h2>
		<h3>Monday and Thursday:</h3>
			<p>10:00 AM to 2:00 PM & 4:30 PM to 8:30 PM</p>
		<h3>Friday:</h3>
			<p>10:00 AM to 2:00 PM</p>

	<h2>Adult ELA Office Hours</h2>
		<h3>Monday - Thursday:</h3>
			<p>9:00 am - 3:00 pm & 4:30 pm - 8:00 pm</p>
	<h2>Provo Adult Education Contacts</h2>
		
		
	<ul class="sociallinks">
			<li><a href="https://www.facebook.com/provoadulted/"><img src="//globalassets.provo.edu/image/icons/facebook.svg" alt="Link to Facebook" /></a></li>
			<li><a href="https://www.instagram.com/provoadulted/"><img src="//globalassets.provo.edu/image/icons/instagram.svg" alt="Link to Instagram" /></a></li>
			<li><a href="https://twitter.com/ProvoAdultEd"><img src="//globalassets.provo.edu/image/icons/twitter.svg" alt="Link to Twitter" /></a></li>
	</ul>
	
-->
</aside>