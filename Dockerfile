...
RUN echo 'RemoteIPHeader X-Forwarded-For' > /etc/apache2/conf-available/remoteip.conf && \
    echo 'RemoteIPTrustedProxy 158.91.1.103' >> /etc/apache2/conf-available/remoteip.conf && \
    a2enconf remoteip
...
# Copy the default .htaccess file to a location outside the container for management
